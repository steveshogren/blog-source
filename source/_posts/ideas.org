...
* Querying Github API with Haskell
I decided to start the next revision of my safety score. This time, however, I
decided to do it in Haskell. Can't be too hard right?

## The Fever Dream

### Step one - Stack

I build a stack site, 
...
Now I need to query the GitHub API. Not my first time to this rodeo, I go
generate a personal access token from GitHub and copy it to a local file. What
query should I run first? How about the count for all ASM tetris repositories?
Poking around [the
docs](https://developer.github.com/v3/search/#search-repositories) comes up
with:
...
Hrm. Doesn't compile. Durp, hackage is a package library, I need to add this to
my cabal. What is the name of the package? HTTP-4000? HTTP-4000.3.2? Ah, just
HTTP. Ok. I update my cabal file... in all three build-depends...?
...
Further inspection of the docs shows a line in paragraph 5. 

``` NOTE: This package only supports HTTP;```

Wat. Ok, I search again. ```haskell https get``` comes up with "http-conduit".
Let's try this bad boy out. After adding http-conduit to my cabal:
...
Huzzah! Results! I'm getting back a monster string of data. Now how to parse
this. Aeson seems to be the biggest contender. To use Aeson, I needed the
following additions:
...
That ResultCount will allow me to use ```decode``` from aeson to parse the
"total_count" from the JSON response into an Int. Sure enough, it does!
...
totally different count. Hrm. Maybe the query request isn't correct? Adding a
```print request``` after building the request shows: 
...
Crap, that queryString isn't right!
...
specs, it dawns on me. "+" is an encoded space. I change my query to ```("q",
Just "tetris+language:assembly")```
and it works!
